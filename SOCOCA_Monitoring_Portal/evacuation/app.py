...
with st.sidebar:
    st.markdown("<div class='brand'><span class='brand-icon'>✦</span><div>EVACUATION CENTER<small>Management System</small></div></div>", unsafe_allow_html=True)
    ...
    st.divider()
    if authenticated:
        st.markdown(f"<div class='user-chip'>👤 <span><small>{escape(role.replace('_', ' ').title())}</small><br>{escape(username)}</span></div>", unsafe_allow_html=True)
        if st.button("🚪 Log out", type="primary", use_container_width=True, key="sidebar_logout"): logout()
